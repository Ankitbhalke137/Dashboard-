...
export default function LeaderboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-headline-lg font-mono text-on-surface pt-12">Global Rankings</h1>
      </div>
          ...
          {[...podium, ...rest].map((entry, idx) => (
            <div key={entry._id} className="flex items-center gap-4 p-4 hover:bg-white/[0.02] transition-all group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-on-surface-variant font-mono">{entry.studentId?.batch ? `${entry.studentId.batch}-${parseInt(entry.studentId.batch) + 1}` : ''}</div>
              </div>
